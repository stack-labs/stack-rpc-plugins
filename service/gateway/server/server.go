...
var (
	_ server.Server = &gatewayServer{}
)

func newServer(opts ...server.Option) *gatewayServer {
...
func (s *gatewayServer) Options() server.Options {
	s.Lock()
	defer s.Unlock()
...
func (s *gatewayServer) Init(opts ...server.Option) error {
	s.Lock()
	defer s.Unlock()
...
func (s *gatewayServer) Handle(h server.Handler) error {
	s.Lock()
	defer s.Unlock()
...
func (s *gatewayServer) NewHandler(h interface{}, opts ...server.HandlerOption) server.Handler {
...
func (s *gatewayServer) NewSubscriber(topic string, fn interface{}, opts ...server.SubscriberOption) server.Subscriber {
...
func (s *gatewayServer) Register() error {
	var err error
...
func (s *gatewayServer) Deregister() error {
	var err error
	var advt, host, port string
...
func (s *gatewayServer) Subscribe(sub server.Subscriber) error {
	s.Lock()
	defer s.Unlock()
...
func (s *gatewayServer) String() string {
	return "gateway"
}
